...
| 2 | "Export an **official city ESG report** (PDF) for council/DENR/CCC." | 🟡 (on‑screen only) | I report to officials and agencies. | Add **ESG report export** (branded PDF/CSV). | 🟠 | M |
| 3 | "**Charts/graphs** of emissions and diversion trends over time." | ❌ | Visuals communicate progress to non‑technical officials. | Add **trend charts** to the LGU dashboard. | 🟢 | S |
| 4 | "Track **community‑based projects** in my jurisdiction beyond endorsing." | 🟡 | I 'track community projects' per my role. | Add an **LGU project tracker** (projects in my municipality + status). | 🟠 | M |
| 5 | "Upload supporting **data files** (waste hauler logs, MRF records)." | ❌ | Evidence backs my numbers. | Add **evidence upload** to LGU records. | 🟢 | S |
| 6 | "Compare against **other LGUs / benchmarks**." | ❌ | Motivates improvement; useful for grants. | Add **benchmarking** (opt‑in, anonymized). | 🟢 | M |
...
## 🎯 Top 3 to build first (biggest value for LGUs)
1. **City ESG report export** (#2) — LGUs need a document to hand to officials/agencies.
2. **LGU project tracker** (#4) — completes the "track community projects" capability.
3. **Trend charts** (#3) — quick, high‑impact visualization win.
